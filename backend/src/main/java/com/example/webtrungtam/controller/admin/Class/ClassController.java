...
public class ClassController {
    ...
    public ResponseEntity<String> createClass(@RequestBody @Valid CreateClassRequest request) {

        // Log dữ liệu nhận vào
        System.out.println("Received Request: " + request);

        // Xử lý tạo lớp
        ClassEntity classEntity = managerClassService.createClass(request);

        classDetailService.generateClassSchedule(classEntity.getIdClass(),
    ...
    // lấy danh sách
    @GetMapping("/class/{id}/list-students")
    public ResponseEntity<List<Student>> getStudentsByClassId(@PathVariable int id) {
        List<Student> students = managerClassService.getStudentsByClassId(id);
